...
def run_mode(cfg):
    ...
    seed = cfg.get("seed", 0)
    # The real (and signed-real) wiring is deterministic from the connectome +
    # neurotransmitter data: it must NOT be resampled per seed. Seed variation
    # for real-graph modes lives in init/training/sampling only. The random
    # control's WIRING is part of its randomness, so it is resampled.
    gseed = seed if mode in ("rand_rand", "fly_rand") else 0
    ...
    indptr, indices, values, N = build_mode(mode, gseed)
...
def main():
    t0 = time.time()
    if SMOKE:
        run_mode({"mode": "flysigned_fly", "train_frac": 1.0, "steps": 3,
                  "seed": 1, "tag": "smoke"})
        run_mode({"mode": "rand_rand", "train_frac": 0.2, "steps": 3,
                  "seed": 1, "tag": "smoke"})
        log(f"SMOKE DONE in {time.time()-t0:.0f}s")
        return
    # Seed replicates of the DECISIVE PAIR from the v3 plastic battery
    # (experiment.md 10.2): flysigned_fly (real+E/I, fully trained) vs
    # rand_rand (matched random control). Per-mode JSONs land in /kaggle/working
    # as each run finishes, so a session timeout preserves completed seeds.
    ST = 3000
    run_mode({"mode": "flysigned_fly", "train_frac": 1.0, "steps": ST, "seed": 1, "tag": "repl"})
    run_mode({"mode": "rand_rand", "train_frac": 0.2, "steps": ST, "seed": 1, "tag": "repl"})
    run_mode({"mode": "flysigned_fly", "train_frac": 1.0, "steps": ST, "seed": 2, "tag": "repl"})
    run_mode({"mode": "rand_rand", "train_frac": 0.2, "steps": ST, "seed": 2, "tag": "repl"})
    with open(f"{OUT}/plastic_battery_summary.json", "w") as f:
        json.dump({"total_wall_seconds": time.time() - t0, "device": DEVICE}, f, indent=2)
    log(f"ALL REPL RUNS DONE in {(time.time()-t0)/60:.1f} min")
