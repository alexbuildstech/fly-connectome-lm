"""Regime scan v3: 16 (in-gain, gain) configs as parallel columns; metric = ridge NLL
for NEXT-char prediction in the exact 64->16-stream training protocol."""
...
configs = [(ig, g) for ig in [0.5, 1.0, 2.0, 4.0] for g in [0.5, 1.0, 1.5, 2.0]]
gains = torch.tensor([g for _, g in configs], dtype=torch.float32).unsqueeze(0)
encs = [torch.from_numpy((np.random.default_rng(100 + k).standard_normal((V, 30000)) * ig).astype(np.float32))
...
S = torch.stack(F_list, dim=0).double().numpy()   # (time, B, 2048)
Ys = torch.stack(y_list, dim=0).numpy()           # (time, B)
